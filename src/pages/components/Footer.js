...
export default function Footer(props) {
	...
	let checkOrder = useSelector(state => state.footerReducer);
	const dispatch = useDispatch();

	async function checkOrders() {
		var response = await verifyEmailToken(true);
		//alert(response);
		if (response == 200) {
	...
	useEffect(() => {
		if (checkOrder && !verifiedEmail) {
			checkOrders()
			// dispatch(handleVerifyEmail(false))
		}
	}, [checkOrder])
	...
	// }
	if(verifiedEmail && defaultEmail == ""){
	 	setEmail(verifiedEmail);

	}else if (billing_details && defaultEmail == "") {
		 	setEmail(JSON.parse(billing_details).email);
	}
	...
	async function verifyEmailToken(preventOtpGeneration) {
		try {
			...
			if (res.data && res.status == 200) {
				...
				// setOtpModal(true);
				//alert("")
				localStorage.setItem("verified-email-address",defaultEmail)
				getClientOrders();
		...
		if (defaultEmail != '' && preventOtpGeneration!=true)
			generateOtp();
		return false;
		...
		try {
			...
			if (res.data) {
				...
				if (!checkOrder)
					...
					localStorage.setItem('billing_details', JSON.stringify({ email: defaultEmail }))
					dispatch(handleVerifyEmail(false));
			...
			localStorage.setItem("verified-email-address",null)
		...
		<footer class="footer">
			...
			<div class="container">
				...
				<div class="row" style={{ justifyContent: "center", flexDirection: "column", alignItems: "center" }}>
					<div class="footer-col">
						...
						<ul>
							...
							{!location.pathname.includes("checkout") && <li style={{ cursor: "pointer" }} onClick={async () => { var response = await verifyEmailToken(true); if (response) { } else showOrdersMenu(true) }}>Orders</li>}
  	 		...
  	 		<div class="footer-col">
				...
				<div>
				<h4 className="finria-sans" style={{ color: "#fff", fontWeight: "bold", fontSize: "30px",marginBottom:"60px" }}>Connect with us</h4>

					<ContactUsForm onFooter={true} />
				</div>
			...
			<Modal className='otp-modal' isOpen={ordersMenu}>
				<ModalHeader>Enter your Email address for verification</ModalHeader>
				<ModalBody>
              ...
              <div style={{marginLeft:"23px",marginTop:"23px",marginBottom:"23px"}}>
				...
				<ModalFooter>
					<Button color="primary" onClick={() => {/*getClientOrders() */verifyEmailToken(); }}>{location.pathname.includes("checkout")?"Verify Email":"Show orders"}</Button>
					{!location.pathname.includes("checkout") && <Button onClick={() => showOrdersMenu(false)}>close</Button>}
								...
								{currentOrderData &&
									...
									<div className='ftr order-view-mobile' style={{ marginBottom: "40px", paddingBottom: "40px", borderBottom: "3px solid #d2af47", borderTop: "3px solid #d2af47", background: /*"linear-gradient(45deg, #f3c86d63, #edb8411f)"*/"#fff", padding: "14px", boxShadow: "rgba(0, 0, 0, 0.02) 0px 1px 3px 0px, rgba(27, 31, 35, 0.15) 0px 0px 0px 1px", margin: "10px 10px" }}>
										<h3 className='order-title' style={{
											fontSize: "21px", marginBottom: "20px", paddingBottom: "20px", borderBottom: "1px solid #cac8ee", backgroundColor: "#3534352b", marginTop: "10px",
											padding: "20px", color: "rgb(80 79 79)"
										}}><span style={{ fontWeight: "normal" }}> Order Date: #</span> : ({currentOrderData.date})</h3>
										...
										{currentOrderData.products && currentOrderData.products.map((val, ind2) => {
											return (
												<div style={ind2 % 2 == 0 ? {
													marginBottom: "20px", backgroundColor: "rgb(213 230 239 / 28%)"
													, padding: "10px", boxShadow: "rgba(0, 0, 0, 0.02) 0px 1px 3px 0px, rgba(27, 31, 35, 0.15) 0px 0px 0px 1px"
												} : {
													marginBottom: "20px", backgroundColor: "#e9e9e9"
													, padding: "10px", boxShadow: "rgba(0, 0, 0, 0.02) 0px 1px 3px 0px, rgba(27, 31, 35, 0.15) 0px 0px 0px 1px"
												}}>
													<Row>
														<Col md="5">{val.title}</Col>
				...
				<ModalBody>
					...
					<div style={{ marginLeft: "0px", marginTop: "23px", marginBottom: "23px", overflow: "auto", maxHeight: "520px" }}>
						...
						{orderData && orderData.map((val, ind2) => {
							...
							return (
								<div style={ind2 % 2 == 0 ? {
									margin: "0px 10px", marginBottom: "20px", backgroundColor: "rgb(255 255 255 / 100%)"
									, padding: "10px", boxShadow: "0 2px 15px -3px rgba(0,0,0,0.07),0 10px 20px -2px rgba(0,0,0,0.04)", borderRadius: "10px"
								} : {
									margin: "0px 10px", marginBottom: "20px", backgroundColor: "#fff"
									, padding: "10px", boxShadow: "0 2px 15px -3px rgba(0,0,0,0.07),0 10px 20px -2px rgba(0,0,0,0.04)", borderRadius: "10px"
								}}>
									<Row>
