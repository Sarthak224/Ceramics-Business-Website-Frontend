...
export default function Sidebar(props){
    ...
    return(
     <div className={props.openMenu?"nav-responsive-sidebar sidebar-open":"nav-responsive-sidebar"} >
      <div style={{textAlign:"right",padding:"10px", fontWeight:"bold",fontSize:"19px", cursor:"pointer",marginBottom:"30px"}}  onClick={(e)=>{props.setOpenMenu(false);}}>X</div>
      ...
      <div className="sidebar-item" onClick={()=>props.setOpenMenu(false)}>   <Link className='sidebar-link' to={"/products"}><span className='nav-link1'> {/*<ShoppingBag width={16} height={16} className='nav-icon'/>*/}<i class="fas fa-shopping-bag" style={{color: "#f3f7fe",marginRight:"5px"}}></i><span> Shop</span></span></Link></div>
